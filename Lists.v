...
Theorem bag_theorem : forall n: nat, forall l: bag, count n (add n l) = S (count n l).
Proof.
  assert (forall n: nat, beq_nat n n = true).
  induction n. reflexivity. simpl. trivial.
  induction l.
...
Proof.
  ...
  - intro. simpl. rewrite -> app_nil_r. reflexivity.
  - intro. simpl. rewrite -> IHl1'. rewrite app_assoc. reflexivity.
Qed.
...
Proof.
  ...
  - reflexivity.
  - replace ((h1 :: t1) ++ l2) with (h1 :: t1 ++ l2).
  + destruct h1. simpl. trivial.
...
Proof.
  ...
  induction l as [| h t].
  reflexivity.
  simpl. rewrite <- beq_nat_refl. simpl. trivial.
Qed.
(** [] *)
...
Proof.
  (* FILL IN HERE *)
  intro. destruct x. simpl. rewrite <- beq_nat_refl. reflexivity.
Qed.
(** [] *)
