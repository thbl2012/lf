...
Proof.
  ...
  + simpl in H. rewrite -> plus_comm in H. simpl in H.
    ...
    inversion H. symmetry in H1. apply IHn' in H1. rewrite -> H1. reflexivity.
Qed.
(** [] *)
...
Proof.
  ...
  destruct m. intro. simpl in H. inversion H.
  intro. simpl in H. apply f_equal. apply IHn in H. apply H.
Qed.
(** [] *)
...
Proof.
  ...
  intro. destruct n.
  intro. inversion H.
  simpl. intro. apply IHl. inversion H. reflexivity.
Qed.
(** [] *)
...
Proof.
  (* FILL IN HERE *)
  induction l as [| (x1, x2) t].
  intros. inversion H. reflexivity.
  intros. simpl in H. destruct (split t) as (t1, t2).
  inversion H. simpl.
  assert (combine t1 t2 = t). { apply IHt. reflexivity. }
...
Proof.
  (* FILL IN HERE *)
  intro.
  destruct (f true) eqn:HT, (f false) eqn:HF; destruct b.
...
Proof.
  (* FILL IN HERE *)
  intros.
  destruct (beq_nat n m) eqn:eq_n_m, (beq_nat m n) eqn:eq_m_n.
  - reflexivity.
  - apply beq_nat_true in eq_n_m. rewrite -> eq_n_m in eq_m_n.
    rewrite <- beq_nat_refl in eq_m_n. inversion eq_m_n.
  - apply beq_nat_true in eq_m_n. rewrite -> eq_m_n in eq_n_m.
    rewrite <- beq_nat_refl in eq_n_m. inversion eq_n_m.
  - reflexivity.
Qed.
(** [] *)
...
Proof.
  ...
  rewrite -> H0 in H. rewrite -> H. rewrite <- beq_nat_refl.
  reflexivity.
Qed.
(** [] *)
...
:= forall X (l1 l2: list X) (l: list (X * X)),
    length l1 = length l2
    -> l = combine l1 l2
    -> split l = (l1, l2).
...
(* FILL IN HERE *)
  intro. induction l1.
  - destruct l2.
    + intros. simpl in H0. rewrite -> H0. reflexivity.
    + intros. inversion H.
  - intros. destruct l2.
    + inversion H.
    + inversion H. simpl in H0. destruct l as [|h t].
      inversion H0.
      inversion H0. specialize IHl1 with (l2:=l2) (l:=t). apply IHl1 in H2.
      rewrite <- H4. simpl. rewrite -> H2. reflexivity. apply H4.
Qed.
...
Proof.
  intros. induction l.
  - inversion H.
  - destruct (test x0) eqn:Hx0.
    + simpl in H. rewrite -> Hx0 in H. inversion H. rewrite <- H1. apply Hx0.
    + simpl in H. rewrite -> Hx0 in H. apply IHl. apply H.
Qed. 
  (* FILL IN HERE *)
...
:= match l with
  | [] => true
  | x :: l0 => if test x then forallb test l0 else false
  end.
...
:= match l with
  | [] => false
  | x :: l0 => if test x then true else existsb test l0
  end.

Example test_forallb_1: forallb oddb [1;3;5;7;9] = true. reflexivity. Qed.

Example test_forallb_2: forallb negb [false;false] = true. reflexivity. Qed.

Example test_forallb_3: forallb evenb [0;2;4;5] = false. reflexivity. Qed.

Example test_forallb_4: forallb (beq_nat 5) [] = true. reflexivity. Qed.

Example test_existsb_1: existsb (beq_nat 5) [0;2;3;6] = false. reflexivity. Qed.

Example test_existsb_2: existsb (andb true) [true;true;false] = true. reflexivity. Qed.

Example test_existsb_3: existsb oddb [1;0;0;0;0;3] = true. reflexivity. Qed.

Example test_existsb_4: existsb evenb [] = false. reflexivity. Qed.
...
Proof.
  ...
  - simpl. destruct (test x) eqn:Hx.
    + unfold existsb'. simpl. rewrite -> Hx. simpl. reflexivity.
    + unfold existsb'. simpl. rewrite -> Hx. simpl. apply IHl.
Qed.
