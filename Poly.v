...
Definition mynatnone : option nat := None.
...
Definition filter_even_gt7 (l : list nat) : list nat
  (* REPLACE THIS LINE WITH ":= _your_definition_ ." *)
:= filter (fun n => evenb n && leb 8 n) l.
...
Definition fold_map {X Y: Type} (f: X -> Y) (l: list X) : list Y
  (* REPLACE THIS LINE WITH ":= _your_definition_ ." *)
:= fold (fun (x: X) (fl: list Y) => (f x) :: fl) l nil.
...
Definition prod_uncurry {X Y Z : Type}
  (f : X -> Y -> Z) (p : X * Y) : Z
  (* REPLACE THIS LINE WITH ":= _your_definition_ ." *) := f (fst p) (snd p).
...
:= fun (X: Type) (f: X -> X) (x: X)
   => m (X -> X) (fun (g: X -> X) => (fun (x': X) => n X g x')) f x.
