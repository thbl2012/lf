...
Definition nandb (b1:bool) (b2:bool) : bool
  ...
  := match b1 with | true => negb b2 | false => true end.

Example test_nandb1:               (nandb true false) = true.
(* FILL IN HERE *) simpl. reflexivity. Qed.
Example test_nandb2:               (nandb false false) = true.
(* FILL IN HERE *) auto. Qed.
...
Definition andb3 (b1:bool) (b2:bool) (b3:bool) : bool
  (* REPLACE THIS LINE WITH ":= _your_definition_ ." *)
  := match b1 with | true => match b2 with | true => b3 | false => false end | false => false end.
...
Fixpoint factorial (n:nat) : nat
  (* REPLACE THIS LINE WITH ":= _your_definition_ ." *)
  := match n with | O => S O | S m => mult (factorial m) n end. 
...
Proof.
  intros n. unfold mult. reflexivity.  Qed.
...
(* FILL IN HERE *)
(*
Fixpoint plus_reject (n m : nat): nat :=
  match n with
  | 0 => m
  | S n' => S(plus_reject m n') end.
*)
(** [] *)
...
Fixpoint bin_to_nat (b: bin) : nat :=
  match b with
  | Ob => O
  | S0 c => (bin_to_nat c) + (bin_to_nat c)
  | S1 c => S((bin_to_nat c) + (bin_to_nat c))
  end.

Example test_bin_incr1: incr Ob = S1 Ob.
reflexivity. Qed.

Example test_bin_incr2: incr (S1 Ob) = S0 (S1 Ob).
reflexivity. Qed.

Example test_bin_incr3: incr (incr (incr Ob)) = S1 (S1 Ob).
reflexivity. Qed.

Example test_bin_to_nat1: bin_to_nat (S1 (S0 (S1 Ob))) = 5.
reflexivity. Qed.

Example test_bin_to_nat_incr: bin_to_nat(incr (incr (S1 (S0 (S1 Ob))))) + 1 = bin_to_nat (S0 (S0 (S0 (S1 Ob)))).
reflexivity. Qed.
