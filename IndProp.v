...
| wrong_ev_SS : forall m, wrong_ev m -> wrong_ev (S (S m)).
...
Proof.
  (* FILL IN HERE *)
  intro. exfalso. inversion H. inversion H1. inversion H3.
Qed.
(** [] *)
...
Proof.
 (* FILL IN HERE *)
  intros. split.
  - intro E'n. induction E'n as [ | | n1 n2 IH'1 IH1 IH'2 IH2].
    apply ev_0. apply ev_SS. apply ev_0. apply ev_sum. apply IH1. apply IH2.
  - intro En. induction En as [| n IH IH']. apply ev'_0.
...
Proof.
  (* FILL IN HERE *)
  intros n m Enm En. generalize Enm. induction En.
  - simpl. trivial.
  - intro. simpl in Enm. apply ev_minus2 in Enm. simpl in Enm. apply IHEn; assumption.
Qed.
(** [] *)
...
Proof.
  ...
  assert (ev ((n + m) + (n + p))). {apply (ev_sum (n+m) (n+p)); assumption. }
  rewrite -> plus_assoc in H. rewrite -> (plus_comm (n + m) n) in H.
  rewrite -> (plus_assoc n n m) in H. rewrite <- (plus_assoc (n + n) m p) in H.
  apply (ev_ev__ev (n + n) (m + p)). apply H.
...
Inductive total_relation : nat -> nat -> Prop :=
  | always : forall n m, total_relation n m.

(** [] *)

Theorem total_relation_correct : forall n m, total_relation n m.
Proof.
  intros. apply always.
Qed.
...
Inductive empty_relation : nat -> nat -> Prop :=
  | never : forall n m, False -> empty_relation n m .
(** [] *)

Theorem empty_relation_correct : forall n m, ~(empty_relation n m).
Proof.
  intros. intro. inversion H. assumption.
Qed.
...
Proof.
  (* FILL IN HERE *)
  intros m n o. generalize m n. induction o.
  - intros. inversion H0. rewrite -> H1 in H. inversion H. apply le_n.
  - intros. inversion H0. rewrite <- H1. apply H.
    specialize IHo with m0 n0. apply IHo in H. apply le_S. apply H. apply H2.
Qed.
...
Proof.
  (* FILL IN HERE *)
  intros n m le_n_m.
  induction le_n_m. apply le_n. apply le_S. apply IHle_n_m.
Qed.
...
Proof.
  ...
  apply (le_trans n (S n) m). apply le_S. apply le_n. apply H1.
Qed.
...
Proof.
 ...
 (* FILL IN HERE *)
  ...
  - apply (le_trans (S n1) (S (n1 + n2)) m).
    + replace (S (n1 + n2)) with ((S n1) + n2). apply le_plus_l. reflexivity.
    + apply H.
  - rewrite -> plus_comm in H. apply (le_trans (S n2) (S (n2 + n1)) m).
    + replace (S (n2 + n1)) with ((S n2) + n1). apply le_plus_l. reflexivity.
    + apply H.
Qed.
...
Proof.
  (* FILL IN HERE *)
  induction n. intros. destruct m. apply le_n. apply O_le_n.
  intros. destruct m.
  - inversion H.
...
Proof.
  (* FILL IN HERE *)
  intros n m. 
  generalize n as k. induction m. intros. inversion H. reflexivity.
  destruct k. intro. reflexivity.
  intro. simpl. apply (Sn_le_Sm__n_le_m k m) in H. apply IHm. apply H.
Qed. 
...
Proof.
  (* FILL IN HERE *)
  intros. apply leb_complete in H. apply leb_complete in H0. apply leb_correct.
...
(** - Which of the following propositions are provable?
    ...
    - If we dropped constructor [c5] from the definition of [R],
      ...
      sentence) explain your answer. NO
...
(* FILL IN HERE *)
  ...
  - intro. induction H.
    ...
    + rewrite -> plus_comm. simpl. rewrite <- IHR. rewrite -> plus_comm. reflexivity.
    + simpl in IHR. rewrite plus_comm in IHR. simpl in IHR. inversion IHR. apply plus_comm.
    + rewrite plus_comm. apply IHR.
  - generalize o as p. induction m.
    + intros. simpl in H. rewrite <- H. generalize n. induction n0.
      * apply c1.
      * apply c3. apply IHn0.
    + intros. destruct p as [|p].
      * inversion H.
      * simpl in H. inversion H. apply c2. apply IHm. reflexivity.
Qed.
(** [] *)
...
Inductive subseq : (list nat) -> (list nat) -> Prop :=
  | ss_nil : subseq [ ] [ ]
  | ss_app_r : forall l1 l2 (h : nat), subseq l1 l2 -> subseq l1 (h :: l2)
  | ss_app : forall l1 l2 (h : nat), subseq l1 l2 -> subseq (h :: l1) (h :: l2).

Theorem subseq_refl : forall l, subseq l l.
Proof.
  induction l. apply ss_nil. apply ss_app. apply IHl.
Qed.

Theorem subseq_app : forall l1 l2 l3, subseq l1 l2 -> subseq l1 (l2 ++ l3).
Proof.
  intros. induction H.
  - simpl. induction l3. apply ss_nil. apply ss_app_r. apply IHl3.
  - simpl. apply ss_app_r. apply IHsubseq.
  - simpl. apply ss_app. apply IHsubseq.
Qed.
...
Proof.
  ...
  induction l3.
  - intros l1 l2 H0 H1. inversion H1. rewrite <- H in H0. inversion H0. apply ss_nil.
  - intros l1 l2 H0 H1. inversion H1.
    + apply IHl3 in H0. apply ss_app_r. apply H0. apply H3.
    + rewrite <- H2 in H0. inversion H0.
      * apply IHl3 in H7. apply ss_app_r. apply H7. apply H3.
      * rewrite -> H. apply ss_app. apply (IHl3 l5 l0). apply H7. apply H3.
Qed.
...
Proof.
  (* FILL IN HERE *)
  intros T ss re. induction ss as [|s0 ss].
...
Proof.
  ...
  - generalize dependent s1. induction s2.
    ...
    + simpl. intros. destruct s1 as [|x1 t1].
      * inversion H. inversion H3. rewrite <- H5 in H0. simpl in H0. inversion H0.
      * inversion H. inversion H3. simpl. apply IHs2 in H4. rewrite H4. reflexivity.
  - intros. rewrite H. clear H. clear s1. induction s2 as [|x s].
   + simpl. apply MEmpty.
   + simpl. replace (x :: s) with ([x] ++ s). apply MApp. apply MChar. apply IHs. trivial.
Qed. 
(** [] *)
...
Proof.
  (* FILL IN HERE *)
  intros. induction re.
  - simpl. split. intro. destruct H. inversion H. intro. inversion H.
  - simpl. split. intro. reflexivity. intro. exists [ ]. apply MEmpty.
  - simpl. split. intro. reflexivity. intro. exists [t]. apply MChar.
  - simpl. split.
    + intro. destruct H. inversion H. apply andb_true_iff. split.
      apply IHre1. exists s1. apply H3.
      apply IHre2. exists s2. apply H4.
    + rewrite -> andb_true_iff. intro. destruct H as [H1 H2].
      apply IHre1 in H1. apply IHre2 in H2. destruct H1 as [s1]. destruct H2 as [s2].
  ...
  - simpl. split. intro. reflexivity. intro. exists []. apply MStar0.
Qed.
(** [] *)
...
Proof.
  (* FILL IN HERE *)
  intros T s re. intro Hs. remember (Star re) as Sre. induction Hs.
  - inversion HeqSre.
  - inversion HeqSre.
  - inversion HeqSre.
  - inversion HeqSre.
  ...
  - inversion HeqSre. apply IHHs2 in HeqSre. destruct HeqSre as [ss2]. exists (s1 :: ss2).
    rewrite H0 in Hs1. rewrite H0 in Hs2. clear H0. clear IHHs1. clear IHHs2. clear re0.
    simpl. destruct H. split.
    + rewrite <- H. reflexivity.
    + intros. destruct H1. rewrite <- H1. apply Hs1. apply H0. apply H1.
Qed.
(** [] *)
...
Lemma app_assoc_4 : forall T (l1 l2 l3 l4 : list T),
      (l1 ++ l2 ++ l3) ++ l4 = l1 ++ l2 ++ l3 ++ l4.
Proof.
  intros. rewrite -> (app_assoc T l1 l2 (l3 ++ l4)).
  rewrite -> (app_assoc T (l1 ++ l2) l3 l4). rewrite -> (app_assoc T l1 l2 l3).
  reflexivity.
Qed.
...
Proof.
  ...
  - simpl. intro. rewrite -> app_length in H. apply Nat.add_le_cases in H. destruct H.
    + apply IH1 in H. destruct H as [s3 H]. destruct H as [s4 H]. destruct H as [s5 H].
      destruct H as [H1 H2]. destruct H2 as [H2 H3].
      exists s3. exists s4. exists (s5 ++ s2).
      split.
      * rewrite -> H1. rewrite <- (app_assoc_4 T s3 s4 s5 s2). reflexivity.
      * split. apply H2. intro. rewrite <- (app_assoc_4 T s3 (napp m s4) s5 s2).
        apply MApp. apply H3. apply Hmatch2.
    + apply IH2 in H. destruct H as [s3 H]. destruct H as [s4 H]. destruct H as [s5 H].
      destruct H as [H1 H2]. destruct H2 as [H2 H3].
      exists (s1 ++ s3). exists s4. exists s5.
      split.
      * rewrite -> H1. rewrite -> (app_assoc T s1 s3 (s4 ++ s5)). reflexivity.
      * split. apply H2. intro. rewrite <- (app_assoc T s1 s3 ((napp m s4) ++ s5)).
        apply MApp. apply Hmatch1. apply H3.

  - simpl. intro. remember (pumping_constant re1) as t1.
    remember (pumping_constant re2) as t2. 
    assert (t1 <= length s1).
    { apply (le_trans t1 (t1 + t2) (length s1)). apply le_plus_l. apply H. }
    apply IH in H0. destruct H0 as [s2]. destruct H0 as [s3]. destruct H0 as [s4].
    destruct H0. destruct H1. exists s2. exists s3. exists s4. split.
    + apply H0.
    + split. apply H1. intro. apply MUnionL. apply H2.

  - simpl. intro. remember (pumping_constant re1) as t1.
    remember (pumping_constant re2) as t2. rewrite -> plus_comm in H. 
    assert (t2 <= length s2).
    { apply (le_trans t2 (t2 + t1) (length s2)). apply le_plus_l. apply H. }
    apply IH in H0. destruct H0 as [s1]. destruct H0 as [s3]. destruct H0 as [s4].
    destruct H0. destruct H1. exists s1. exists s3. exists s4. split.
    + apply H0.
    + split. apply H1. intro. apply MUnionR. apply H2.

  - simpl. omega.

  - simpl. rewrite -> app_length. intro.
    destruct s1 as [|x1 t1].
    + simpl in H. exists [ ]. exists s2. exists [ ]. split.
      rewrite app_nil_r. reflexivity.
      split.
      * intro. rewrite H0 in H. inversion H.
      * simpl. intro. rewrite app_nil_r. induction m.
        simpl. apply MStar0. simpl. apply star_app. apply Hmatch2. apply IHm.
    + exists [ ]. exists (x1 :: t1). exists s2. simpl. split.
      reflexivity.
      split.
      * intro. inversion H0.
      * intro. induction m.
        simpl. apply Hmatch2.
        remember (x1 :: t1) as s1. simpl. rewrite <- app_assoc.
        apply MStarApp. apply Hmatch1. apply IHm.
Qed.
...
Proof.
  ...
  - inversion H. split. intro. apply H0 in H1. inversion H1. intro. inversion H1.
Qed.
(** [] *)
...
Proof.
  (* FILL IN HERE *)
  intros n l. induction l as [|m l IHl].
  - simpl. intros. intro. apply H0.
  - simpl. intro. intro. destruct (beq_natP n m).
    + inversion H.
    + simpl in H. apply IHl in H. destruct H0. apply H1. rewrite H0. reflexivity.
                                               apply H. apply H0.
Qed.
(** [] *)
