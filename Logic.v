...
Proof.
  ...
  - left. reflexivity.
  - right. simpl in H. apply and_exercise in H. apply H.
Qed.
(** [] *)
...
Proof.
  (* FILL IN HERE *)
  intros P HP. intros Q. intro. apply ex_falso_quodlibet.
  apply HP. apply H.
Qed.
(** [] *)
...
Proof.
  (* FILL IN HERE *)
  intros. intro. apply H in H1. apply H0. apply H1.
Qed.
(** [] *)
...
Proof.
  (* FILL IN HERE *)
  intros. intro. destruct H as [HP HNP]. apply HNP in HP. apply HP.
Qed.
(** [] *)
...
Proof.
  (* FILL IN HERE *)
  intros. destruct H as [HPQ HQP]. destruct H0 as [HQR HRQ]. split.
  - intro. apply HQR. apply HPQ. apply H.
  - intro. apply HQP. apply HRQ. apply H.
Qed.
(** [] *)
...
Proof.
  ...
  intros. split.
  - intro. split.
    + destruct H as [HP | HQR]. left. apply HP. destruct HQR as [HQ HR]. right. apply HQ.
    + destruct H as [HP | HQR]. left. apply HP. destruct HQR as [HQ HR]. right. apply HR.
  - intro. destruct H as [HPoQ HPoR]. destruct HPoQ as [HP | HQ].
    + left. apply HP.
...
Proof.
  (* FILL IN HERE *)
  intros. intro. destruct H0. apply H0. apply H.
Qed.
(** [] *)
...
Proof.
  ...
  - simpl. split. intro. exfalso. apply H. intro. destruct H. destruct H. apply H0.
  - simpl. split. intro. destruct H.
    exists h. split. apply H. left. reflexivity.
    apply IHt in H. destruct H as [x0 H0]. exists x0. destruct H0.
    split. apply H. right. apply H0.
    intro. destruct H as [x0 H0]. destruct H0. destruct H0.
    left. rewrite H0. apply H.
    right. apply IHt. exists x0. split. apply H. apply H0.
Qed. 
(** [] *)
...
Proof.
  ...
  simpl. split. intro. right. apply H. intro. destruct H. inversion H. apply H.
  simpl. rewrite <- or_assoc. rewrite <- IHt. reflexivity.
Qed.
(** [] *)
...
Proof.
  (* FILL IN HERE *)
  intros. induction l as [|x0 l0].
  simpl. split. intro. reflexivity. intros. inversion H0.
  simpl. split.
  - intro. split.
    + apply H. left. reflexivity.
    + apply IHl0. intros. apply H. right. apply H0.
  - intros. destruct H as [Hx0 HA]. rewrite <- IHl0 in HA. destruct H0.
    + rewrite <- H. apply Hx0.
    + apply HA. apply H.
Qed. 
(** [] *)
...
Lemma rev_append_app_distr : forall X (l1 l2 l: list X),
...
Lemma rev_append_app_assoc : forall X (l1 l2 l: list X),
      (rev_append l1 l2) ++ l = rev_append l1 (l2 ++ l).
Proof.
  intros X l1. induction l1 as [|x l1].
  intros. simpl. reflexivity.
  intros. simpl. apply IHl1.
...
Proof.
  ...
  unfold tr_rev. intro l. induction l.
  reflexivity.
  simpl. assert (rev_append l [x] = (rev_append l [ ]) ++ [x]).
  { rewrite -> rev_append_app_assoc. reflexivity. }
...
Proof.
  ...
  - destruct IHn. exists x.
    rewrite -> evenb_S. rewrite -> Hevn. simpl. rewrite <- H. reflexivity.
  - destruct IHn. exists (S x). rewrite -> evenb_S. rewrite -> Hevn. simpl.
...
Proof.
  (* FILL IN HERE *)
  intros. destruct b1.
  - split. intro. split. reflexivity. simpl in H. apply H.
  intro. apply H.
...
Proof.
  ...
  - split. intro. simpl in H. right. apply H.
    intro. simpl. destruct H. inversion H. apply H.
Qed.
(** [] *)
...
Proof.
  (* FILL IN HERE *)
  split.
  - intro. intro. rewrite -> H0 in H. rewrite <- beq_nat_refl in H. inversion H.
  - intro. destruct (beq_nat x y) eqn:eq_x_y.
    + apply beq_nat_true in eq_x_y. apply H in eq_x_y. inversion eq_x_y.
    + reflexivity.
Qed.
(** [] *)
...
:= match l1, l2 with
  ...
  | [ ], _ => false
  | _, [ ] => false
  | x1 :: t1 , x2 :: t2 => if (beq x1 x2) then (beq_list beq t1 t2) else false
  end.
...
(* FILL IN HERE *)
  ...
  - intro. split.
    + intro. destruct l2. reflexivity. inversion H0.
    + intro. rewrite <- H0. reflexivity.
  - destruct l2 as [|x2 l2].
    + simpl. split. intro. inversion H0. intro. inversion H0.
    + simpl. split.
      * intro. destruct (beq x1 x2) eqn:eq_x1_x2. rewrite -> H in eq_x1_x2.
        rewrite -> IHl1 in H0. rewrite eq_x1_x2. rewrite H0. reflexivity.
        inversion H0.
      * intro. destruct (beq x1 x2) eqn:eq_x1_x2. rewrite IHl1. inversion H0. reflexivity.
        inversion H0. rewrite <- H in H2. rewrite <- H2. rewrite -> eq_x1_x2. reflexivity.
Qed.
(** [] *)
...
Proof.
  (* FILL IN HERE *)
  induction l.
  - simpl. split; intro; reflexivity.
  - simpl. rewrite <- IHl. apply andb_true_iff.
Qed.
