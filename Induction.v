...
Proof.
  (* FILL IN HERE *)
  assert (forall m n: nat, m * S n = m + m * n).
  { intros. induction m.
    reflexivity.
    simpl.
    rewrite -> plus_assoc.
...
Proof.
  (* FILL IN HERE *)
  intro. induction n. reflexivity. simpl. trivial. Qed.
...
Proof.
  (* FILL IN HERE *)
  intro. simpl. rewrite plus_comm. trivial. Qed.
...
Lemma incr_bin_to_nat : forall b: bin, bin_to_nat(incr b) = S(bin_to_nat b).
Proof.
...
Proof.
  induction n.
  simpl. reflexivity.
  simpl. rewrite -> incr_bin_to_nat. rewrite -> IHn. reflexivity.
Qed.


Lemma nat_to_S0_bin : forall n: nat, nat_to_bin(n + n) = S0 (nat_to_bin n).
Proof.
  induction n. simpl. Abort.


Theorem bin_to_nat_to_bin : forall b: bin, nat_to_bin(bin_to_nat b) = b.
Proof.
  induction b. reflexivity.
  simpl. Abort.
